...
function ColumnResize(table) {
	...
	for (var i=0; i<dragColumns.length; i++) {
	    var tdColumn = $(dragColumns[i]);
	    var Column = tdColumn.find('colunm-header');
	    if(Column.parent().hasClass('resize_column_wrapper')){
	        tdColumn.find('.resize_column_element').remove();
	        Column.unwrap();
	    }
	    
	    var tableHeight = Column.closest("table").height();
	    
	    Column.wrap("<div class='resize_column_wrapper' style='position:relative;height:100%;width:100%'></div>");
	    
	    var draggg = $("<div class='resize_column_element' style='position:absolute;height:"+tableHeight+"px;width:5px;margin-right:-5px;left:100%;top:0px;cursor:w-resize;z-index:10;'> </               div>").on( "mousedown", this.startColumnDrag);
	    
	    Column.before(draggg); 
	 }   
	 
	 	/*dragColumns[i].innerHTML = ""+
			"<div class='resize_column_element' style='"+
...
function RemoveResizableColumns()
{
    $('th').each(function(){
        var tdColumn = $(this);
        var Column = tdColumn.find('colunm-header');
